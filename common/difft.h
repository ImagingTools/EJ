...
void *memCopy(void *dest, const void *src, size_t n); // { return memcpy(dest, src, n); }
void *memSet(void *dest, int c, size_t n); // { return memset(dest, c, n); }
...
////    return lcsr(a.left(i), b.left(k), buff) + lcsr(a.right(m-i), b.right(n-k), buff);
//    res = lcsr(a.left(i), b.left(k), buff) + lcsr(a.right(m-i), b.right(n-k), buff);
//    return res;
////    return lcsr(a.left(i), b.left(k), buff) + lcsr(a.right(m-i), b.right(n-k), buff);
//}
...
template<typename T>
//QList<T> lcs(const QList<T> &a, const QList<T> &b)
//{
//    // Optimization: preallocate vectors to avoid memory relocation
//    QVector<int> L1(b.length()+1);
//    QVector<int> L2(b.length()+1);
//    QVector<int> t(b.length()+1);

//    int *buff[] = {L1.data(), L2.data(), t.data()};

//    return lcsr(QListRef<T>(&a), QListRef<T>(&b), buff);
//}
...
//    QList<T> t = lcs(a, b);
    ...
    for(typename QList<T>::const_iterator pk = t.constBegin(); pk != tEnd; pk++) {
        ...
        i1 = i2;
        //        while(!equals(*i2, *pk) && i2 != aEnd) {
        while(i2 != aEnd && !equals(*i2, *pk)) {
            i2++;
        }

        j1 = j2;
        //        while(!equals(*j2, *pk) && j2 != bEnd) {
        while(j2 != bEnd && !equals(*j2, *pk)) {
            j2++;
        }

//        //        while(j2 - j1 > i2 - i1 && (i2+1) != aEnd && equals(*(i2+1), *pk)) {
//        while(i2 != aEnd && j2 - j1 > i2 - i1 && (i2+1) != aEnd && equals(*(i2+1), *pk)) {

//            i2++;
//        }
//        //        while(i2 - i1 > j2 - j1 && (j2+1) != bEnd && equals(*(j2+1), *pk)) {
//        while(j2 != bEnd && i2 - i1 > j2 - j1 && (j2+1) != bEnd && equals(*(j2+1), *pk)) {
//            j2++;
//        }
...
//void writePatchRecord(QDataStream &stream, const int &i1, const QByteArray &s1, const int &i2, const QByteArray &s2, bool rev = true)
//{
//    IntVarLenConst n2(i2);
//    IntVarLenConst n1(i1);
//    ByteArrVarLenConst bal1(s1, rev);
//    ByteArrVarLenConst bal2(s2);
//    stream << n1 << bal1 << n2 << bal2;
//}

/**
 * @brief patch Applies difference patch to the byte array. If invert is true,
 * the function works in reversed order, i.e.
 *   patch(a, diff(a,b), false) == b
 *   patch(b, diff(a,b), true) == a
 */
template<typename T>
QList<T> patch(const QList<T> &orig, const QByteArray &diff, bool invert = false)
{
    QList<T> result;
